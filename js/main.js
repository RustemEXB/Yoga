...
function checkForm(event) {
	...
	if(name == "" || mail == "" || tel == "" || subject == "" || message == "")
...
// Сброс данных при отправке формы
function clearForm() {
	let inputs = document.querySelectorAll('.active__input');
	inputs.forEach(input => {
		input.value='';
	})
}